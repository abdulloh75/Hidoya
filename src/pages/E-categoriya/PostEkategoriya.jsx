...
function PostEkategoriya({ handleClick, fetchData }) {
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      toast.success("Electron kategoriya qo'shildi", {
      ...
      setIsModalOpen(false);
        ...
        console.error(error);
      toast.error("Electron kategoriya qo'shilmadi", {
        position: toast.POSITION.TOP_RIGHT,
      });
    }finally {
      setLoading(false);
        ...
        <Modal className="md:mt-52" width={380} open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
          <form className="md:block" onSubmit={handleSubmit(onSubmit)}>
            <div className="items-center md:flex">
              <div className="mb-4">
                <label htmlFor="icon" className=" ml-0 block text-gray-700 text-sm font-bold mb-2">Electron kategoriya qo'shish</label>
                <input
                  type="text"
                  placeholder="Electron kategoriya nomi"
...
export default PostEkategoriya;
